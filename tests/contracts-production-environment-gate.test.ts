...
describe("contract production environment gate", () => {
  test("mirrors the canonical Commerce QA signals", () => {
    for (const signal of [
      ...
      expect(migration.replace(/\s+/g, "")).toContain(signal.replace(/\s+/g, ""));
    }
  ...
  test("filters readiness and contract-party UI to production orders", () => {
    expect(migration.match(/order_matches_commerce_environment\(o\.tenant_id,o\.id,'production'\)/g)?.length).toBeGreaterThanOrEqual(3);
    expect(migration).toContain("Leitura de prontidão de produção");
    expect(migration).toContain("public checkout sem cobrança de produção");
  });
