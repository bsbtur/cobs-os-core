...
const migration = readFileSync("supabase/migrations/20260907161000_citytour_r1_test_checkout_v1.sql", "utf8");
...
describe("City Tour R$1 TEST Golden Path", () => {
  it("resolves the QA fixture by stable key and freezes the value at 100 minor", () => {
    expect(migration).toContain("citytour-r1-contract-gates-v1");
  ...
  it("connects the landing only to TEST-specific functions", () => {
    expect(landing).toContain('functions.invoke("citytour-test-checkout"');
    expect(landing).toContain('functions.invoke("citytour-test-create-pix"');
    expect(landing).not.toContain('functions.invoke("ciosp-public-create-pix"');
