...
Deno.serve(async (req: Request) => {
  ...
  if (insertError) {
    ...
    console.error("chapada_commercial_lead_insert_failed", insertError);
  ...
  if (eventError) {
    console.error("chapada_lead_automation_event_insert_failed", eventError);
  ...
  let webhook: URL;
  try {
    webhook = new URL(N8N_COMMERCIAL_WEBHOOK_URL.trim());
  ...
  try {
    ...
    await admin.from("audit_events").insert({ tenant_id: op.tenant_id, actor_profile_id: null, action: "automation.dispatch", subject_type: "automation_event", subject_id: automationEvent.id, correlation_id: correlationId, metadata: { event_type: "lead.created", orchestrator: "n8n", source: "chapada_public" } });
