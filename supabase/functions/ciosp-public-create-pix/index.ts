...
Deno.serve(async (req: Request) => {
  ...
  if (MP_ENVIRONMENT !== "test") return json({ error: "public_pix_test_only" }, 503);
  if (!MP_TEST_ACCESS_TOKEN) return json({ error: "mercado_pago_test_not_configured" }, 500);
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payerEmail)) return json({ error: "valid_payer_email_required" }, 400);

  const admin = createClient(SUPABASE_URL, secretKey, { auth: { persistSession: false } });
  const tokenHash = await sha256(token);
  const nowIso = new Date().toISOString();
  const { data: session, error: sessionError } = await admin.from("public_checkout_sessions")
  ...
  if (new Date(session.expires_at).getTime() <= Date.now()) {
    await admin.from("public_checkout_sessions").update({ status: "expired", updated_at: nowIso }).eq("id", session.id);
    return json({ error: "checkout_session_expired" }, 409);
  }

  const { data: order, error: orderError } = await admin.from("orders")
    .select("id,tenant_id,operation_id,status,currency,grand_total_minor,reference_label,metadata")
  ...
  const { data: operation } = await admin.from("operations").select("code,offering_id,archived_at").eq("id", order.operation_id).maybeSingle();
  if (!operation || operation.code !== CANONICAL_CIOSP_OPERATION_CODE || operation.archived_at) return json({ error: "order_not_canonical_ciops" }, 409);

  const { data: offering } = await admin.from("offerings").select("id,status,metadata").eq("id", operation.offering_id).maybeSingle();
  ...
  const qaClosedSalesAllowed = orderMeta.qa_public_checkout === true && MP_ENVIRONMENT === "test";
  if (offeringMeta.sales_public !== true && !qaClosedSalesAllowed) return json({ error: "sales_not_open" }, 409);
  ...
  const paidBefore = Math.max((gross - reversed) - refunded, 0);
  const total = Number(order.grand_total_minor ?? 0);
  const outstanding = Math.max(total - paidBefore, 0);
  if (!Number.isFinite(outstanding) || outstanding <= 0) return json({ error: "order_has_no_outstanding_balance" }, 409);

  const configuredEntry = Number(offeringMeta.entry_minor ?? 0);
  const stage = paidBefore <= 0 && configuredEntry > 0 && configuredEntry < total ? "entry" : "balance";
  const amountMinor = stage === "entry" ? Math.min(configuredEntry, outstanding) : outstanding;
  if (!Number.isFinite(amountMinor) || amountMinor <= 0) return json({ error: "invalid_charge_amount" }, 409);

  await admin.rpc("ensure_order_reservation_for_payment", { _order_id: order.id });

  const { data: openCharges, error: openChargeError } = await admin.from("payment_charges")
    .select("id,tenant_id,order_id,status,currency,amount_minor,external_reference,provider,provider_order_id,metadata")
    .eq("order_id", order.id).eq("provider", "mercado_pago")
    .in("status", ["draft", "pending", "processing"]).order("created_at", { ascending: false }).limit(10);
  if (openChargeError) return json({ error: "charge_lookup_failed" }, 500);
  let charge = (openCharges ?? []).find((c: any) => Number(c.amount_minor) === amountMinor && c?.metadata?.commercial_payment_stage === stage) ?? null;
  ...
  if (!charge) {
    ...
    const { data: createdCharge, error: createChargeError } = await admin.from("payment_charges").insert({
      ...
      description: stage === "entry" ? "CIOSP 2027 — entrada" : "CIOSP 2027 — saldo",
      created_by: null,
      metadata: { environment: "test", source: "public_checkout", commercial_payment_stage: stage, paid_before_minor: paidBefore, entry_minor: configuredEntry },
    }).select("id,tenant_id,order_id,status,currency,amount_minor,external_reference,provider,provider_order_id,metadata").single();
  ...
  const { data: existingAttempt, error: attemptLookupError } = await admin.from("payment_attempts").select("*")
    .eq("charge_id", charge.id).eq("provider", "mercado_pago").eq("method", "pix")
    .in("status", ["created", "pending", "processing"]).order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  if (existingAttempt?.provider_order_id && (existingAttempt.pix_qr_code || existingAttempt.pix_ticket_url)) {
    return json({
      order_id: order.id, charge_id: charge.id, attempt_id: existingAttempt.id,
      amount_minor: amountMinor, stage, status: existingAttempt.status,
  ...
  if (!attempt) {
    ...
    const { data: createdAttempt, error: createAttemptError } = await admin.from("payment_attempts").insert({
      tenant_id: order.tenant_id, charge_id: charge.id, provider: "mercado_pago", method: "pix", status: "created",
      amount_minor: amountMinor, idempotency_key: idempotencyKey, request_snapshot: requestSnapshot,
      metadata: { payment_method_id: "pix", environment: "test", source: "public_checkout" },
    }).select("*").single();
  ...
  try {
    mpResponse = await fetch("https://api.mercadopago.com/v1/orders", {
      method: "POST",
      headers: { accept: "application/json", "content-type": "application/json", authorization: `Bearer ${MP_TEST_ACCESS_TOKEN}`, "x-idempotency-key": attempt.idempotency_key },
  ...
  if (!mpResponse.ok) {
    ...
    return json({ error: "mercado_pago_error", status: mpResponse.status, attempt_id: attempt.id }, 502);
  }
  ...
  await admin.from("payment_attempts").update({
    status: attemptStatus, provider_order_id: mp?.id ?? null, provider_payment_id: payment?.id ?? null,
    provider_status: providerStatus, provider_status_detail: providerStatusDetail,
    pix_qr_code: method?.qr_code ?? null, pix_qr_code_base64: method?.qr_code_base64 ?? null,
    pix_ticket_url: method?.ticket_url ?? null, response_snapshot: mp,
    ...(attemptStatus === "approved" ? { approved_at: now } : {}),
  }).eq("id", attempt.id);
  ...
  await admin.from("payment_charges").update({
    status: chargeStatus, provider_order_id: mp?.id ?? null,
    ...(chargeStatus === "paid" ? { paid_amount_minor: amountMinor, paid_at: now } : {}),
  }).eq("id", charge.id);

  if (chargeStatus === "paid") {
    const { error: factError } = await admin.rpc("record_provider_payment", {
      _order_id: order.id, _amount_minor: amountMinor,
      _reference: `mercado_pago:${payment?.id ?? mp?.id}`,
      _reason: stage === "entry" ? "CIOSP 2027 public checkout entry approved" : "CIOSP 2027 public checkout balance approved",
    ...
    await admin.from("public_checkout_sessions").update({ status: "consumed", updated_at: now }).eq("id", session.id);
  }

  return json({
    order_id: order.id, charge_id: charge.id, attempt_id: attempt.id,
    provider_order_id: mp?.id ?? null, provider_payment_id: payment?.id ?? null,
    amount_minor: amountMinor, stage, status: attemptStatus,
    pix: { qr_code: method?.qr_code ?? null, qr_code_base64: method?.qr_code_base64 ?? null, ticket_url: method?.ticket_url ?? null },
    reused: false, environment: "test",
  }, 201);
});
