...
Deno.serve(async (req: Request) => {
  ...
  const mercadoPagoToken = MP_ENVIRONMENT === "test" ? MP_TEST_ACCESS_TOKEN : MP_ACCESS_TOKEN;
  if (!mercadoPagoToken) {
    return json(
      {
        error:
          MP_ENVIRONMENT === "test"
            ? "mercado_pago_test_not_configured"
  ...
  const { data: operation } = await admin
    .from("operations")
    .select("code,offering_id,archived_at")
    .eq("id", order.operation_id)
    .maybeSingle();

  if (!operation || operation.code !== CANONICAL_CIOSP_OPERATION_CODE || operation.archived_at) {
    return json({ error: "order_not_canonical_ciops" }, 409);
  }

  const { data: offering } = await admin
    .from("offerings")
    .select("id,status,metadata")
    .eq("id", operation.offering_id)
    .maybeSingle();

  if (!offering || offering.status !== "active") {
    return json({ error: "offering_not_active" }, 409);
  }
  ...
  if (offeringMeta.sales_public !== true && !qaClosedSalesAllowed) {
    return json({ error: "sales_not_open" }, 409);
  }
  ...
  const stage =
    paidBefore <= 0 && configuredEntry > 0 && configuredEntry < total ? "entry" : "balance";
  const amountMinor = stage === "entry" ? Math.min(configuredEntry, outstanding) : outstanding;
  if (!Number.isFinite(amountMinor) || amountMinor <= 0) {
    return json({ error: "invalid_charge_amount" }, 409);
  }

  const { error: reservationError } = await admin.rpc("ensure_order_reservation_for_payment", {
    _order_id: order.id,
  });
  if (reservationError) return json({ error: "reservation_prepare_failed" }, 500);

  const { data: openCharges, error: openChargeError } = await admin
  ...
  let charge =
    (openCharges ?? []).find(
      (candidate: any) =>
        Number(candidate.amount_minor) === amountMinor &&
        candidate?.metadata?.commercial_payment_stage === stage,
    ) ?? null;
    ...
    const { data: createdCharge, error: createChargeError } = await admin
      ...
      .insert({
        ...
        metadata: {
          ...
          commercial_payment_stage: stage,
          paid_before_minor: paidBefore,
          entry_minor: configuredEntry,
        },
      })
      .select("id,tenant_id,order_id,status,currency,amount_minor,external_reference,provider,provider_order_id,metadata")
      .single();
  ...
  const chargeStatus =
    attemptStatus === "approved" ? "paid" : attemptStatus === "processing" ? "processing" : "pending";
  ...
  if (chargeStatus === "paid") {
    const { error: factError } = await admin.rpc("record_provider_payment", {
      ...
      _reason:
        stage === "entry"
          ? "CIOSP 2027 public checkout entry approved"
          : "CIOSP 2027 public checkout balance approved",
