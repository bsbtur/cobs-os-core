...
Deno.serve(async (req: Request) => {
  ...
  if (!["test", "production"].includes(MP_ENVIRONMENT))
  ...
  if (!requestedChargeId && !orderId)
    return json({ error: "charge_id_or_order_id_required" }, 400);
  ...
  async function loadOrder(id: string, tenantId?: string): Promise<OrderEnvironmentSource | null> {
    let query = admin
      .from("orders")
    ...
    return data as OrderEnvironmentSource | null;
  }

  let charge: any = null;
  let order: OrderEnvironmentSource | null = null;
  let paymentEnvironment = MP_ENVIRONMENT;
  ...
  if (requestedChargeId) {
    const { data, error } = await admin
      ...
      .eq("id", requestedChargeId)
      .maybeSingle();
    if (error) return json({ error: "charge_lookup_failed", details: error.message }, 500);
    if (!data) return json({ error: "charge_not_found" }, 404);
    charge = data;
    try {
      order = await loadOrder(charge.order_id, charge.tenant_id);
    } catch (error) {
      return json({ error: "order_lookup_failed", details: String(error) }, 500);
    }
    if (!order) return json({ error: "order_not_found" }, 404);
    const resolved = await resolvePaymentEnvironment(order);
    if ("error" in resolved) return json({ error: resolved.error }, 409);
    paymentEnvironment = resolved.environment;
    mpAccessToken = resolved.accessToken;
    if (charge?.metadata?.environment !== paymentEnvironment)
      return json({ error: "charge_environment_mismatch", expected_environment: paymentEnvironment }, 409);
  } else {
    try {
    ...
    const { data: membership, error: membershipError } = await admin
      ...
      .maybeSingle();
    if (membershipError)
      return json({ error: "membership_check_failed", details: membershipError.message }, 500);
    ...
    if (factsError)
      return json({ error: "financial_facts_lookup_failed", details: factsError.message }, 500);
    ...
    const outstandingMinor = Math.max(
      Number(order.grand_total_minor ?? 0) - ((gross - reversed) - refunded),
      0,
    );
    if (!Number.isFinite(outstandingMinor) || outstandingMinor <= 0)
      return json({ error: "order_has_no_outstanding_balance" }, 409);

    const { data: chargeCandidates, error: existingChargeError } = await admin
      .from("payment_charges")
      .select("id,tenant_id,order_id,status,currency,amount_minor,external_reference,provider,provider_order_id,metadata")
      .eq("order_id", order.id)
      .eq("tenant_id", order.tenant_id)
    ...
    if (existingChargeError)
      return json({ error: "charge_lookup_failed", details: existingChargeError.message }, 500);
    const existingCharge = (chargeCandidates ?? []).find(
      (candidate: any) => candidate?.metadata?.environment === paymentEnvironment,
    );
    if (existingCharge) {
      charge = existingCharge;
    } else {
      const externalReference = `cobs_${String(order.id).replaceAll("-", "")}_${Date.now()}`;
      const { data: createdCharge, error: createChargeError } = await admin
        .from("payment_charges")
        .insert({
          ...
          metadata: { environment: paymentEnvironment, source: "commerce_order" },
        })
        .select("id,tenant_id,order_id,status,currency,amount_minor,external_reference,provider,provider_order_id,metadata")
        .single();
      if (createChargeError || !createdCharge)
        return json({ error: "charge_create_failed", details: createChargeError?.message }, 500);
      charge = createdCharge;
    }
  }

  if (!mpAccessToken) return json({ error: "mercado_pago_not_configured" }, 500);
  ...
  const { data: attemptCandidates, error: existingError } = await admin
    ...
    .limit(20);
  if (existingError)
    return json({ error: "attempt_lookup_failed", details: existingError.message }, 500);
  const existingAttempt = (attemptCandidates ?? []).find(
    (candidate: any) => candidate?.metadata?.environment === paymentEnvironment,
  ) ?? null;
  ...
  if (existingAttempt?.provider_order_id) {
    try {
      const currentResponse = await fetch(
        `https://api.mercadopago.com/v1/orders/${encodeURIComponent(existingAttempt.provider_order_id)}`,
        { headers: { accept: "application/json", authorization: `Bearer ${mpAccessToken}` } },
      );
      ...
      if (currentResponse.ok) {
        ...
        const chargeStatus = attemptStatus === "approved"
          ? "paid"
  ...
  if (!attempt) {
    ...
    const payer = paymentEnvironment === "test"
    ...
    const { data: createdAttempt, error: createAttemptError } = await admin
      ...
      .insert({
        ...
        metadata: { payment_method_id: "pix", environment: paymentEnvironment },
      })
      .select("*")
      .single();
    if (createAttemptError || !createdAttempt)
      return json({ error: "attempt_create_failed", details: createAttemptError?.message }, 500);
  ...
  if (updateAttemptError)
    return json({ error: "attempt_update_failed", details: updateAttemptError.message, provider_order_id: mp?.id ?? null }, 500);
  ...
  if (updateChargeError)
    return json({ error: "charge_update_failed", details: updateChargeError.message, provider_order_id: mp?.id ?? null }, 500);
