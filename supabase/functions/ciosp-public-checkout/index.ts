...
function isQaReferer(req: Request) {
  const referer = req.headers.get("referer");
  if (!referer) return false;
  try {
    const url = new URL(referer);
    return url.pathname === "/ciosp-2027" && url.searchParams.get("sales_qa") === "1";
  } catch {
    return false;
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  const checkoutKey = (body.checkout_key ?? "commercial") as keyof typeof CHECKOUTS;
  ...
  const { data: op, error: opError } = await admin.from("operations")
    .select("id,tenant_id,code,offering_id,archived_at")
    .eq("code", cfg.operationCode).is("archived_at", null).maybeSingle();
  if (opError) return json({ error: "operation_lookup_failed" }, 500);
  if (!op?.offering_id) return json({ error: "checkout_not_configured" }, 409);

  const { data: offering, error: offeringError } = await admin.from("offerings")
    .select("id,status,metadata").eq("id", op.offering_id).eq("tenant_id", op.tenant_id).maybeSingle();
  ...
  let qaClosedSalesAllowed = false;
  const salesClosed = cfg.requiresPublicSales && (offering.status !== "active" || metadata.sales_public !== true);
  if (salesClosed) {
    if (!isQaReferer(req)) return json({ error: "sales_not_open" }, 409);
    ...
    if (!authHeader) return json({ error: "qa_auth_required" }, 401);
    ...
    qaClosedSalesAllowed = true;
  }

  const { data: sellable } = await admin.from("sellables").select("id")
    .eq("tenant_id", op.tenant_id).eq("offering_id", offering.id).eq("status", "active").limit(1).maybeSingle();
  if (!sellable) return json({ error: "active_sellable_not_found" }, 409);

  const now = new Date().toISOString();
  const { data: price } = await admin.from("prices").select("id")
    .eq("tenant_id", op.tenant_id).eq("sellable_id", sellable.id).eq("status", "active")
    .lte("valid_from", now).or(`valid_until.is.null,valid_until.gt.${now}`).limit(1).maybeSingle();
  ...
  const { data, error } = await admin.rpc("create_public_checkout_order", {
    ...
    _allow_closed: qaClosedSalesAllowed,
  });
  if (error) {
  ...
  if (qaClosedSalesAllowed && data?.order_id) {
    const { data: orderRow } = await admin.from("orders").select("metadata").eq("id", data.order_id).maybeSingle();
    const orderMetadata = (orderRow?.metadata ?? {}) as Record<string, unknown>;
    await admin.from("orders").update({
      metadata: {
        ...orderMetadata,
        qa_public_checkout: true,
        qa_environment: "test",
        qa_sales_public_bypass: true,
      },
    }).eq("id", data.order_id);
  }
