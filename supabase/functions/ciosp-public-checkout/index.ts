...
Deno.serve(async (req: Request) => {
  ...
  if (data?.reused === true && data?.session_id && data?.order_id) { const { data: rotatedSession, error: rotateError } = await admin.from("public_checkout_sessions").update({ token_hash: tokenHash, status: "active", expires_at: new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString() }).eq("id", data.session_id).eq("order_id", data.order_id).eq("tenant_id", op.tenant_id).select("id").maybeSingle(); if (rotateError || !rotatedSession) return json({ error: "checkout_session_rotation_failed" }, 409); }
