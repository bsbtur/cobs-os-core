-- P1 #248: staff journey reminders are only useful until their milestone.
-- Keep the message persisted/auditable, but let existing inbox projections
-- naturally suppress it once expires_at <= now().
  ...
  loop
    ...
    begin
      ...
      insert into public.messages (
        tenant_id,operation_id,kind,priority,status,title,body,locale,published_at,expires_at,
        created_by,published_by,recipient_count,in_app_reachable_count,metadata
      )
      values (
        _candidate.tenant_id,_candidate.operation_id,'reminder'::public.message_kind,
        'normal'::public.message_priority,'published',_title,_body,'pt-BR',now(),
        _candidate.milestone_at,auth.uid(),auth.uid(),1,1,
        jsonb_build_object(
