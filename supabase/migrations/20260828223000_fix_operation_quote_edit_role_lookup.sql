...
begin
  ...
  if not app_private.has_tenant_role(v_quote.tenant_id,array['owner','admin']::public.app_role[]) then raise exception 'permission_denied'; end if;
  ...
  if nullif(trim(_category),'') is null then raise exception 'category_required'; end if;
  if nullif(trim(_description),'') is null then raise exception 'description_required'; end if;
  ...
  values (v_quote.tenant_id,trim(_supplier_name),trim(_category))
  on conflict (tenant_id,name) do update set category=excluded.category, updated_at=now()
  returning id into v_supplier_id;
  update public.operation_quotes set supplier_id=v_supplier_id, category=trim(_category), description=trim(_description),
    amount_minor=_amount_minor, valid_until=_valid_until, cancellation_terms=nullif(trim(coalesce(_cancellation_terms,'')),''), notes=nullif(trim(coalesce(_notes,'')),''), updated_at=now()
...
revoke all on function public.delete_operation_quote(uuid) from public, anon;
