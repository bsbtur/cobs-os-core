...
begin
  ...
  select coalesce(sum(pc.paid_amount_minor),0),coalesce(sum(pc.refunded_amount_minor),0),coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object('status',pc.status::text,'amount_minor',pc.amount_minor,'paid_amount_minor',pc.paid_amount_minor,'refunded_amount_minor',pc.refunded_amount_minor,'installment_number',pc.installment_number,'installment_count',pc.installment_count,'due_date',case when pc.due_at is not null then (pc.due_at at time zone coalesce((select timezone from public.operations where id=_operation_id and tenant_id=_tenant_id),'UTC'))::date else null end,'paid_date',case when pc.paid_at is not null then (pc.paid_at at time zone coalesce((select timezone from public.operations where id=_operation_id and tenant_id=_tenant_id),'UTC'))::date else null end)) order by pc.installment_number nulls last,pc.created_at),'[]'::jsonb) into _paid_total,_refunded_total,_charges from public.payment_charges pc where pc.tenant_id=_tenant_id and pc.order_id=_order_id and pc.status::text<>'cancelled';
  _net_paid:=greatest(_paid_total-_refunded_total,0);
  _balance:=greatest(coalesce(_order_total,0)-_net_paid,0);
  return jsonb_build_object('currency',_currency,'amount_unit','minor','order_total_minor',coalesce(_order_total,0),'paid_total_minor',_paid_total,'refunded_total_minor',_refunded_total,'net_paid_minor',_net_paid,'balance_due_minor',_balance,'payment_status',case when _balance=0 and coalesce(_order_total,0)>0 then 'paid' when _net_paid>0 then 'partially_paid' when jsonb_array_length(_charges)>0 then 'pending' else 'no_charge' end,'charges',_charges);
end;
$$;
