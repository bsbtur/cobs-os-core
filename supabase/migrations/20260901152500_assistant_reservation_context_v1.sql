...
begin
  ...
  select jsonb_strip_nulls(jsonb_build_object(
    ...
    'package_name', oi.sellable_name_snapshot,
    'quantity', r.quantity,
  ...
  order by
    case r.status::text when 'confirmed' then 0 else 1 end,
...
begin
  ...
  values (
    _c.tenant_id, _c.operation_id, _c.profile_id, 'assistant.request', 'cobs_app',
    _idem, 'assistant:' || _c.id::text || ':' || _message_id::text,
    jsonb_build_object(
  ...
  update public.assistant_conversation_messages
     set automation_event_id = _event_id, status = 'pending'
