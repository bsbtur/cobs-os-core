create or replace function app_private.assistant_build_trusted_context(_tenant_id uuid, _operation_id uuid, _profile_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select * into _grant from public.participant_access_grants g
  ...
  select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
    ...
    'expected_start',j.expected_start,'expected_end',j.expected_end,'location',j.location_label
  )) order by j.sequence),'[]'::jsonb) into _journey
  from public.journey_steps j where j.tenant_id=_tenant_id and j.operation_id=_operation_id
  ...
  from public.transport_legs t where t.tenant_id=_tenant_id and t.operation_id=_operation_id;
  ...
  if _grant.participation_id is not null then
    select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
      ...
      'planned_check_out',s.planned_check_out,'expected_check_out',s.expected_check_out,'room',r.label
    ...
    where sp.tenant_id=_tenant_id and sp.participation_id=_grant.participation_id and sp.is_active is true and sp.removed_at is null;
  end if;

  select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
    'name',e.name,'status',e.status::text,'timezone',e.timezone,'schedule_precision',e.schedule_precision,
    ...
    'planned_start_date',case when e.schedule_precision='date_only' and e.planned_start is not null then (e.planned_start at time zone coalesce(e.timezone,_o.timezone,'UTC'))::date else null end,
    'planned_end_date',case when e.schedule_precision='date_only' and e.planned_end is not null then (e.planned_end at time zone coalesce(e.timezone,_o.timezone,'UTC'))::date else null end,
  ...
  from public.events e where e.tenant_id=_tenant_id and e.operation_id=_operation_id;
  ...
  return jsonb_build_object(
    ...
    'reservation','{}'::jsonb,'payment','{}'::jsonb,
    'schedule',jsonb_build_object('journey',_journey,'transport',_transport,'events',_events),
    'hospitality',_hospitality,'documents','{}'::jsonb,'known_facts',_known);
end;
$function$;