-- CIOSP 2027 Commercial Golden Path · P0-04
-- Materialize identified beneficiaries from a fully confirmed commerce order
-- into the linked operation without duplicating participants.
...
begin
  ...
  if _o.status <> 'confirmed' then
    raise exception 'Only a confirmed order can materialize operation participations';
  end if;
  if _o.operation_id is null then
    return jsonb_build_object(
      ...
      'participation_ids', '[]'::jsonb,
      'skipped', 'order_has_no_operation'
    );
  end if;
  ...
  loop
    select * into _existing
     ...
     where p.operation_id = _o.operation_id
       and p.person_id = _beneficiary_id
     for update;
    ...
    if _existing.id is null then
      insert into public.operation_participations(
        ...
        'confirmed'::public.participation_status,
        now(),
        null
      )
      ...
      perform app_private.record_audit_event(
        _o.tenant_id,
        null,
        'commerce.participation_materialized_from_paid_order',
        'operation_participation',
        _participation_id,
        null,
        jsonb_build_object(
          'order_id', _order_id,
          'operation_id', _o.operation_id,
          'person_id', _beneficiary_id
        )
      );
      ...
      perform app_private.record_audit_event(
        _o.tenant_id,
        null,
        'commerce.participation_confirmed_from_paid_order',
        'operation_participation',
        _participation_id,
        null,
      ...
      -- A cancelled participation is an explicit operational decision.
      -- A late/replayed payment webhook must not silently reactivate it.
  ...
  return jsonb_build_object(
    ...
    'cancelled_conflicts', _cancelled_conflicts,
    'participation_ids', to_jsonb(_ids)
  );
end;
$function$;

revoke all on function public.materialize_paid_order_participations(uuid)
  from public, anon, authenticated;
grant execute on function public.materialize_paid_order_participations(uuid)
  to service_role;

create or replace function public.confirm_paid_provider_order(
  _order_id uuid,
  _charge_id uuid,
...
begin
  ...
  update public.orders
     set status='confirmed', confirmed_at=now(), confirmed_by=null,
         metadata = coalesce(metadata,'{}'::jsonb) || jsonb_build_object(
           'provider_confirmation',jsonb_build_object(
             'provider','mercado_pago',
             'charge_id',_charge_id,
             'reference',_provider_reference,
             'confirmed_at',now()
           )
         )
   where id=_order_id;
  ...
  perform app_private.record_audit_event(
    _o.tenant_id,
    null,
    'commerce.order_confirmed_by_provider',
    'order',
    _order_id,
    null,
    jsonb_build_object(
...
revoke all on function public.confirm_paid_provider_order(uuid,uuid,text)
  from public, anon, authenticated;
grant execute on function public.confirm_paid_provider_order(uuid,uuid,text)
  to service_role;
