...
BEGIN
  SELECT id, offering_id
    INTO _operation_id, _offering_id
  FROM public.operations
  WHERE code = 'CIOSP-SP-2027'
  ...
  IF _sellable_id IS NULL THEN
    RAISE EXCEPTION 'Active CIOSP 2027 sellable not found';
  END IF;

  SELECT id
    INTO _price_id
  FROM public.prices
  WHERE sellable_id = _sellable_id
    AND active = true
    AND currency = 'BRL'
  ORDER BY created_at DESC
  LIMIT 1;

  IF _price_id IS NULL THEN
    RAISE EXCEPTION 'Active CIOSP 2027 BRL price not found';
  END IF;
