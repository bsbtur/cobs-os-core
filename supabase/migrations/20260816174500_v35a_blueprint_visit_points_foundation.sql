-- V3.5A — Versioned Visit Points inside Journey Blueprints
...
create policy "Members read blueprint visit points"
  ...
  using (
    app_private.has_tenant_role(
      tenant_id,
      array['owner','admin','operations_agent','member']::public.app_role[]
    )
  );
...
begin
  ...
  if tg_op = 'UPDATE' and (
    ...
    new.blueprint_step_id is distinct from old.blueprint_step_id or
    new.tenant_id is distinct from old.tenant_id
  ) then
    raise exception 'A blueprint visit point cannot move between steps, versions or organisations';
  end if;

  if tg_op = 'INSERT' and new.tenant_id <> _version.tenant_id then
...
begin
  ...
  if _step.id is null then raise exception 'Blueprint step not found'; end if;
  _version := app_private.blueprint_version_ctx(_step.version_id, array['owner','admin','operations_agent']);
  if _version.status <> 'draft' then raise exception 'Only a draft version can receive visit points'; end if;
  ...
  select coalesce(max(sequence), 0) + 10 into _sequence
  from public.journey_blueprint_visit_points
  where blueprint_step_id = _step.id;

  perform set_config('app.blueprint_control', 'on', true);
  insert into public.journey_blueprint_visit_points (
    tenant_id, version_id, blueprint_step_id, sequence, title, interpretation, guide_tip, created_by
  ) values (
    _version.tenant_id, _version.id, _step.id, _sequence, _title_clean,
  ...
  perform app_private.record_audit_event(
    ...
    jsonb_build_object('version_id', _version.id, 'blueprint_step_id', _step.id, 'sequence', _row.sequence)
  );
  insert into public.idempotency_keys (tenant_id, actor_profile_id, action, idempotency_key, result)
  values (_version.tenant_id, auth.uid(), 'blueprint.visit_point_add', _key, _existing);
  return _existing;
end;
$function$;
...
begin
  ...
  _version := app_private.blueprint_version_ctx(_row.version_id, array['owner','admin','operations_agent']);
  if _version.status <> 'draft' then raise exception 'Only a draft version can change visit points'; end if;
  ...
  perform set_config('app.blueprint_control', 'on', true);
  update public.journey_blueprint_visit_points
  ...
  perform app_private.record_audit_event(
    ...
    'journey_blueprint_visit_point', _row.id, _key,
    jsonb_build_object('version_id', _version.id, 'blueprint_step_id', _row.blueprint_step_id)
  );
  insert into public.idempotency_keys (tenant_id, actor_profile_id, action, idempotency_key, result)
  values (_version.tenant_id, auth.uid(), 'blueprint.visit_point_update', _key, _existing);
  return _existing;
end;
$function$;
...
begin
  ...
  if _row.id is null then raise exception 'Blueprint visit point not found'; end if;
  _version := app_private.blueprint_version_ctx(_row.version_id, array['owner','admin','operations_agent']);
  if _version.status <> 'draft' then raise exception 'Only a draft version can remove visit points'; end if;
  ...
  _existing := jsonb_build_object('blueprint_visit_point_id', _row.id, 'removed', true);
  perform app_private.record_audit_event(
  ...
  insert into public.idempotency_keys (tenant_id, actor_profile_id, action, idempotency_key, result)
  values (_version.tenant_id, auth.uid(), 'blueprint.visit_point_remove', _key, _existing);
  return _existing;
end;
$function$;
...
begin
  ...
  if _step.id is null then raise exception 'Blueprint step not found'; end if;
  _version := app_private.blueprint_version_ctx(_step.version_id, array['owner','admin','operations_agent']);
  if _version.status <> 'draft' then raise exception 'Only a draft version can reorder visit points'; end if;
  ...
  select count(*) into _total from public.journey_blueprint_visit_points where blueprint_step_id = _step.id;
  select count(distinct x) into _given from unnest(coalesce(_ordered_visit_point_ids, '{}'::uuid[])) x;
  if _given <> coalesce(array_length(_ordered_visit_point_ids, 1), 0) then raise exception 'Visit point list contains duplicates'; end if;
  if _given <> _total then raise exception 'Ordered list must contain every visit point exactly once'; end if;
  if exists (
  ...
  ) then raise exception 'Ordered list contains a visit point from another step'; end if;

  if _total > 0 then
    select coalesce(max(sequence), 0) + 1000 into _shift
    from public.journey_blueprint_visit_points where blueprint_step_id = _step.id;
    perform set_config('app.blueprint_control', 'on', true);
    update public.journey_blueprint_visit_points set sequence = sequence + _shift where blueprint_step_id = _step.id;
    for _i in 1 .. array_length(_ordered_visit_point_ids, 1) loop
      update public.journey_blueprint_visit_points set sequence = _i * 10 where id = _ordered_visit_point_ids[_i];
    end loop;
    perform set_config('app.blueprint_control', 'off', true);
  end if;

  _existing := jsonb_build_object('blueprint_step_id', _step.id, 'visit_point_count', _total);
  perform app_private.record_audit_event(
    _version.tenant_id, auth.uid(), 'journey_blueprint_visit_point.reordered',
    'journey_blueprint_step', _step.id, _key, jsonb_build_object('visit_point_count', _total)
  );
  insert into public.idempotency_keys (tenant_id, actor_profile_id, action, idempotency_key, result)
  values (_version.tenant_id, auth.uid(), 'blueprint.visit_point_reorder', _key, _existing);
  return _existing;
end;
$function$;
...
as $function$
  with canonical as (
    select s.sequence as step_sequence, 0 as point_sequence,
      concat_ws('|', 'STEP', s.sequence::text, btrim(s.title), s.step_kind::text,
        ...
        coalesce(s.presence_requirement::text,'default'), s.presence_population::text) as line
    from public.journey_blueprint_steps s where s.version_id = _version_id
    union all
    select s.sequence, p.sequence,
      concat_ws('|', 'POINT', s.sequence::text, p.sequence::text, btrim(p.title),
        coalesce(btrim(p.interpretation),''), coalesce(btrim(p.guide_tip),''))
