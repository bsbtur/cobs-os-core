...
BEGIN
  ...
  LEFT JOIN app_private.rpc_security_allowlist_v1 a ON a.schema_name=n.nspname AND a.function_name=p.proname AND a.identity_args=pg_get_function_identity_arguments(p.oid)
  WHERE p.prosecdef AND n.nspname IN ('public','app_private') AND a.schema_name IS NULL;
  IF v_unlisted <> 64 THEN RAISE EXCEPTION 'rpc_allowlist_expected_64_unlisted_found_%', v_unlisted; END IF;

  SELECT count(*) INTO v_anon_unlisted
  FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
  LEFT JOIN app_private.rpc_security_allowlist_v1 a ON a.schema_name=n.nspname AND a.function_name=p.proname AND a.identity_args=pg_get_function_identity_arguments(p.oid)
  WHERE p.prosecdef AND n.nspname IN ('public','app_private') AND a.schema_name IS NULL AND has_function_privilege('anon',p.oid,'EXECUTE');
  IF v_anon_unlisted <> 4 THEN RAISE EXCEPTION 'rpc_allowlist_expected_4_anon_unlisted_found_%', v_anon_unlisted; END IF;
END
$do$;
...
(schema_name,function_name,identity_args,classification,expected_authenticated,expected_anon,expected_service_role,captured_at)
SELECT n.nspname,p.proname,pg_get_function_identity_arguments(p.oid),
  CASE
    WHEN n.nspname='app_private' AND p.proname IN ('assistant_can_access_conversation','assistant_has_operation_access') THEN 'ALLOW'
  ...
  CASE
    WHEN n.nspname='public' AND NOT has_function_privilege('authenticated',p.oid,'EXECUTE') THEN has_function_privilege('service_role',p.oid,'EXECUTE')
...
BEGIN
  ...
  IF v_issues <> 0 THEN RAISE EXCEPTION 'rpc_security_allowlist_gate_failed_with_%_issues', v_issues; END IF;
END
$do$;
