...
-- Append-only audit trail for schedule lifecycle and rescheduling.
...
-- Read access follows tenant membership. Mutations are RPC-only.
...
-- No INSERT/UPDATE/DELETE grants or policies for authenticated users.
-- Canonical writes happen only through SECURITY DEFINER commands below.
...
-- Read-only conflict projection for UI and QA. It does not bypass RLS.
