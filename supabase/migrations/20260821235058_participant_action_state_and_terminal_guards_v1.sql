...
begin
  ...
  select o.status into _op_status from public.operations o
   where o.id = _row.operation_id and o.tenant_id = _row.tenant_id;
...
begin
  ...
  select pa.status into _part_status from public.operation_participations pa
   where pa.id = _g.participation_id and pa.tenant_id = _g.tenant_id;
  if _part_status = 'cancelled' then
    raise exception 'A cancelled participation cannot regain portal access';
  end if;
  select o.status into _op_status from public.operations o
   where o.id = _g.operation_id and o.tenant_id = _g.tenant_id;
  if _op_status = 'cancelled' then
    raise exception 'Portal access cannot be reinstated for a cancelled operation';
  end if;
