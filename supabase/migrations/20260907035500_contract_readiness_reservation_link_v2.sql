-- COBS OS · Contract readiness reservation link v2
-- Corrects the commercial reservation relation: reservations belong to orders, not directly to operations.
-- Still read-only: no legal, supplier, contract, provider or payment state is mutated.
...
begin
  ...
  with contractable_orders as (
    ...
    left join public.offerings off
      on off.tenant_id=co.tenant_id and off.id=cr.offering_id
  ), schedule_eval as (
  ...
  return jsonb_build_object(
    ...
    'checks',jsonb_build_array(
      ...
      jsonb_build_object('key','commercial_terms','label','Pedidos contratáveis com termos comerciais versionados','status',case when v_order_count>0 and v_terms_ready_count=v_order_count then 'ready' else 'blocked' end,'kind','technical','detail',v_terms_ready_count||'/'||v_order_count||' completos'),
      ...
      jsonb_build_object('key','payment_schedule','label','Plano de pagamento congelado e compatível com cada pedido','status',case when v_order_count>0 and v_payment_ready_count=v_order_count then 'ready' else 'blocked' end,'kind','technical','detail',v_payment_ready_count||'/'||v_order_count||' compatíveis')
    ),
...
comment on function public.get_operation_contract_readiness(uuid,text) is
  'Read-only contract readiness projection v2. Resolves offering evidence through commercial_reservations.order_id and keeps provider send locked.';
