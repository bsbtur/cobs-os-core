-- W10 portal claim hotfix
-- A valid invitation opened under the wrong authenticated account must not be
-- reported as an invalid token. Return a structured, non-consuming outcome so
-- the frontend can guide the traveler to use the invited account without
-- weakening token, tenant or Person/Profile protections.
