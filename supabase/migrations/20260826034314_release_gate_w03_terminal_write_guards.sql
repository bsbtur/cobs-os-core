CREATE OR REPLACE FUNCTION public.add_operation_participation(_operation_id uuid, _person_id uuid, _participation_kind participation_kind, _idempotency_key text, _role_type_ids uuid[] DEFAULT '{}'::uuid[], _primary_role_type_id uuid DEFAULT NULL::uuid, _notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.assign_operation_role(_participation_id uuid, _role_type_id uuid, _is_primary boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
CREATE OR REPLACE FUNCTION public.set_primary_operation_role(_participation_id uuid, _role_type_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
CREATE OR REPLACE FUNCTION public.unassign_operation_role(_participation_id uuid, _role_type_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
