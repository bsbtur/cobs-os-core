create table if not exists public.public_checkout_sessions (
...
alter table public.public_checkout_sessions enable row level security;
revoke all on public.public_checkout_sessions from anon, authenticated;
grant select, insert, update on public.public_checkout_sessions to service_role;

create index if not exists public_checkout_sessions_order_idx on public.public_checkout_sessions(order_id);
create index if not exists public_checkout_sessions_expires_idx on public.public_checkout_sessions(expires_at) where status='active';
...
begin
  ...
  select * into _op from public.operations o
   where o.code=btrim(_operation_code) and o.archived_at is null
   limit 1;
  ...
  select * into _off from public.offerings o
   where o.id=_op.offering_id and o.tenant_id=_op.tenant_id;
  ...
  values(_op.tenant_id,_op.id,_person_id,btrim(_full_name),'BRL','draft','CIOSP 2027 — checkout público',jsonb_build_object('source','public_checkout','public_checkout_idempotency_key',btrim(_idempotency_key)),null)
  returning id into _order_id;

  insert into public.order_items(
    tenant_id,order_id,sellable_id,price_id,offering_id,sellable_kind,sellable_name_snapshot,description_snapshot,
    price_basis,currency,unit_amount_minor,quantity,discount_minor,line_subtotal_minor,line_total_minor,beneficiary_person_id,created_by
  ) values(
    _op.tenant_id,_order_id,_sell.id,_price.id,_sell.offering_id,_sell.sellable_kind,_off.name,coalesce(_price.description,_sell.description),
    _price.price_basis,'BRL',_price.unit_amount_minor,1,0,_price.unit_amount_minor,_price.unit_amount_minor,_person_id,null
...
revoke all on function public.create_public_checkout_order(text,text,text,text,text,text,boolean) from public, anon, authenticated;
grant execute on function public.create_public_checkout_order(text,text,text,text,text,text,boolean) to service_role;