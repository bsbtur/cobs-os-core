...
begin
  ...
  if _existing.id is not null then
    if _allow_closed and coalesce((_existing.metadata->>'qa_public_checkout')::boolean,false) is not true then
      ...
      update public.orders
         set metadata = coalesce(metadata,'{}'::jsonb) || jsonb_build_object('qa_public_checkout',true,'qa_environment','test','qa_sales_public_bypass',true)
  ...
  if _allow_closed then
    _order_metadata := _order_metadata || jsonb_build_object('qa_public_checkout',true,'qa_environment','test','qa_sales_public_bypass',true);
  end if;
  ...
  values(_op.tenant_id,_op.id,_person_id,btrim(_full_name),'BRL','draft','CIOSP 2027 — checkout público',_order_metadata,null)
