...
-- Internal in-app reminders derived from canonical staff assignments.
-- No WhatsApp, SMS, email or external push in V1.

create unique index if not exists ux_messages_staff_alert_key
  on public.messages ((metadata ->> 'staff_alert_key'))
  where metadata ? 'staff_alert_key';

create or replace function public.generate_due_staff_journey_alerts(
  _tenant_id uuid,
  _window_start timestamptz default (now() - interval '5 minutes'),
...
begin
  ...
  -- Human-triggered runs require the existing communication operator authorization.
  -- Backend/service-role schedulers are allowed to run the same canonical generator.
  ...
  for _candidate in
    with milestones as (
      select
        ...
        op.timezone as operation_timezone,
        rt.label as role_label,
        'report_at'::text as milestone,
        a.report_at as milestone_at,
        a.report_at - interval '15 minutes' as alert_at
      ...
      select
        a.id, a.tenant_id, a.operation_id, a.participation_id, a.role_type_id,
  ...
  loop
    -- V1 is in-app only: do not generate unreachable reminders.
    if not exists (
      select 1
      from app_private.w08_in_app_eligible_recipients(
        _candidate.tenant_id,
        _candidate.operation_id,
        array[_candidate.person_id]::uuid[]
      ) e
      where e.person_id = _candidate.person_id
    ) then
    ...
    _alert_key := format(
      'staff-alert:%s:%s:%s',
      _candidate.assignment_id,
      _candidate.milestone,
    ...
    if exists (
      ...
      where m.tenant_id = _tenant_id
        and m.metadata ->> 'staff_alert_key' = _alert_key
    ) then
    ...
    _time_label := to_char(
      _candidate.milestone_at at time zone coalesce(nullif(_candidate.operation_timezone,''), 'America/Sao_Paulo'),
      'HH24:MI'
    );
    ...
    begin
      ...
      insert into public.messages (
        tenant_id, operation_id, kind, priority, status, title, body, locale,
        published_at, created_by, published_by, recipient_count, in_app_reachable_count,
        metadata
      ) values (
        _candidate.tenant_id,
        _candidate.operation_id,
        'reminder'::public.message_kind,
        'normal'::public.message_priority,
        'published',
        _title,
        _body,
        'pt-BR',
        ...
        1,
        jsonb_build_object(
          'source', 'px12_staff_journey_alert',
          'staff_alert_key', _alert_key,
          'staff_assignment_id', _candidate.assignment_id,
          'milestone', _candidate.milestone,
          'milestone_at', _candidate.milestone_at,
          'alert_at', _candidate.alert_at,
          'role_type_id', _candidate.role_type_id
        )
      ...
      insert into public.message_audience_selectors (
        ...
        _candidate.tenant_id, _message.id, 'explicit_person', _candidate.person_id, auth.uid()
      );

      insert into public.message_recipients (
        tenant_id, message_id, person_id, in_app_eligible
      ) values (
        _candidate.tenant_id, _message.id, _candidate.person_id, true
      ...
      insert into public.message_deliveries (
        ...
        _candidate.tenant_id, _message.id, _recipient_id, _candidate.person_id,
      ...
      perform app_private.w08_record_communication_event(
        ...
        _recipient_id,
        _delivery_id,
        jsonb_build_object('channel','in_app','source','px12_staff_journey_alert'),
        _correlation
      );
      ...
      perform app_private.record_audit_event(
        _candidate.tenant_id,
        auth.uid(),
        'px12.staff_alert_generated',
        'message',
        _message.id,
        _alert_key,
        jsonb_build_object(
          'assignment_id', _candidate.assignment_id,
          'person_id', _candidate.person_id,
          'milestone', _candidate.milestone,
          'milestone_at', _candidate.milestone_at
        )
      );
  ...
  return jsonb_build_object(
    ...
    'skipped_ineligible', _skipped_ineligible
  );
end;
$$;

revoke all on function public.generate_due_staff_journey_alerts(uuid,timestamptz,timestamptz) from public;
grant execute on function public.generate_due_staff_journey_alerts(uuid,timestamptz,timestamptz) to authenticated;
grant execute on function public.generate_due_staff_journey_alerts(uuid,timestamptz,timestamptz) to service_role;

comment on function public.generate_due_staff_journey_alerts(uuid,timestamptz,timestamptz) is
  'PX12.5 V1: idempotently materializes due staff schedule reminders into the canonical W07 in-app message domain.';
