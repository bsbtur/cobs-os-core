...
declare
  ...
  _execution_block_code text := null;
  _execution_block_label text := null;
begin
  ...
  if _current is not null then
    _execution_block_code := 'STEP_ALREADY_ACTIVE';
    _execution_block_label := 'Conclua ou pule a etapa ativa antes de iniciar a próxima.';
  elsif _next is null then
    _execution_block_code := 'NO_NEXT_STEP';
    _execution_block_label := 'Não há próxima etapa disponível para iniciar.';
  elsif _op.status in ('draft','planning') then
    _execution_block_code := 'OPERATION_NOT_READY';
    _execution_block_label := 'A operação ainda não está pronta para execução. Avance o ciclo até Pronta.';
  elsif _op.status in ('completed','cancelled') then
    _execution_block_code := 'OPERATION_TERMINAL';
    _execution_block_label := 'A operação já foi encerrada e não aceita novas ações operacionais.';
  end if;
  ...
  return jsonb_build_object(
    ...
    'execution_block_code', _execution_block_code,
    'execution_block_label', _execution_block_label,
