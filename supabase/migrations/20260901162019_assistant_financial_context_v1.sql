...
begin
  ...
  select o.id,o.currency,o.grand_total_minor
  into _order_id,_currency,_order_total
  ...
  where r.tenant_id=_tenant_id
    and o.operation_id=_operation_id
    and oi.beneficiary_person_id=_person_id
    and r.status::text in ('confirmed','reserved')
    and o.status::text<>'cancelled'
  order by case r.status::text when 'confirmed' then 0 else 1 end,
  ...
  select
    coalesce(sum(pc.paid_amount_minor),0),
    coalesce(sum(pc.refunded_amount_minor),0),
    coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
      'status',pc.status::text,
      'amount_minor',pc.amount_minor,
      'paid_amount_minor',pc.paid_amount_minor,
      'refunded_amount_minor',pc.refunded_amount_minor,
      'installment_number',pc.installment_number,
  ...
  into _paid_total,_refunded_total,_charges
  ...
  return jsonb_build_object(
    'currency',_currency,
    'amount_unit','minor',
    'order_total_minor',coalesce(_order_total,0),
    'paid_total_minor',_paid_total,
    'refunded_total_minor',_refunded_total,
    'net_paid_minor',_net_paid,
    'balance_due_minor',_balance,
    'payment_status',case
      when _balance=0 and coalesce(_order_total,0)>0 then 'paid'
      when _net_paid>0 then 'partially_paid'
      when jsonb_array_length(_charges)>0 then 'pending'
      else 'no_charge'
    end,
    'charges',_charges
  );
