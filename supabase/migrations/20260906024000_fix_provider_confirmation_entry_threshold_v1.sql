...
begin
  ...
  if coalesce(_c.metadata->>'source','')='public_checkout'
     and coalesce((_c.metadata->>'schedule_aware')::boolean,false)=true
     and coalesce(_c.metadata->>'commercial_payment_stage','')='entry'
     and coalesce(_c.installment_number,0)=1
     and coalesce(_c.amount_minor,0)>0 then
    _entry_minor := _c.amount_minor;
  else
    select min(nullif((ofr.metadata->>'entry_minor')::bigint,0))
      into _entry_minor
      from public.order_items oi
      join public.offerings ofr on ofr.id=oi.offering_id and ofr.tenant_id=oi.tenant_id
     where oi.order_id=_order_id
       and coalesce((ofr.metadata->>'sales_public')::boolean,false)=true;
  end if;
  ...
  if _paid < _required_paid then raise exception 'Order has not reached confirmation payment threshold'; end if;

  if _o.status='confirmed' then
    _participations := public.materialize_paid_order_participations(_order_id);
    return jsonb_build_object('order_id',_order_id,'status','confirmed','unchanged',true,'paid_minor',_paid,'confirmation_threshold_minor',_required_paid,'participations',_participations);
  end if;
  ...
  perform app_private.record_audit_event(_o.tenant_id,null,'commerce.order_confirmed_by_provider','order',_order_id,null,jsonb_build_object('charge_id',_charge_id,'provider','mercado_pago','reference',_provider_reference,'paid_minor',_paid,'confirmation_threshold_minor',_required_paid));
  _participations := public.materialize_paid_order_participations(_order_id);
