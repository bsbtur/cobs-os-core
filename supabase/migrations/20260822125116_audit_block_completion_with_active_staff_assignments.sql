-- AUD-008: prevent terminal operations from retaining mutable staff responsibilities.
-- Repair the single class of already-impossible historical rows before enforcing the gate.

alter table public.operation_staff_assignments disable trigger trg_closed_op_staff_assignments;

update public.operation_staff_assignments sa
...
where o.id = sa.operation_id
  and o.tenant_id = sa.tenant_id
  and o.status = 'completed'
  and o.completed_at is not null
  and sa.status in ('assigned','confirmed')
  and sa.ends_at <= o.completed_at;

alter table public.operation_staff_assignments enable trigger trg_closed_op_staff_assignments;
