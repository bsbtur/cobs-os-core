...
begin
  ...
  select * into _step from public.journey_steps s where s.id = _step_id and s.archived_at is null;
...
begin
  ...
  select s.id into _current from public.journey_steps s
   ...
   where s.operation_id = _op.id and s.archived_at is null
     and not exists (select 1 from public.journey_events c where c.journey_step_id = s.id and c.event_type in ('STEP_COMPLETED','STEP_SKIPPED'))
  ...
  select s.id into _next from public.journey_steps s
   where s.operation_id = _op.id and s.archived_at is null
     ...
     and not exists (select 1 from public.journey_events c where c.journey_step_id = s.id and c.event_type in ('STEP_STARTED','STEP_COMPLETED','STEP_SKIPPED'))
   order by s.sequence limit 1;
  return jsonb_build_object('operation_id', _op.id, 'status', _op.status, 'current_step_id', _current, 'next_step_id', _next, 'readiness', case when _current is null then null else public.w04_step_readiness(_current) end);
end;
$function$;
...
create policy "Elevated roles read active journey steps" on public.journey_steps for select to authenticated using (archived_at is null and app_private.has_tenant_role(tenant_id, array['owner','admin','operations_agent']::public.app_role[]));