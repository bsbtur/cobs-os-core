...
-- New writes must never persist inverted windows or blank operator labels.
-- NOT VALID preserves compatibility with historical rows while enforcing the
-- rule for every new/updated row from this migration forward.
...
-- Strengthen the existing Journey baseline guard. From READY onward only the
-- forecast fields may change. Planning metadata, provenance and archive state
-- are historical truth and must not be rewritten by any client/RPC.
...
-- Playbook items are planning truth. Runtime execution belongs exclusively to
-- playbook_executions. Do not let checklist definitions silently change after
-- the operation reaches READY.
...
-- Explicit least-privilege posture for the trigger helper.
...
-- Keep the existing journey guard non-invokable as an RPC surface too.
