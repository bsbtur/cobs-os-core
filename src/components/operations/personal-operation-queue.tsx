...
export function PersonalOperationQueue() {
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<QueueItem[]> => {
      ...
      const participations = await supabase
        ...
        .select(
          "id,operation_id,status,operations(id,name,code,status,planned_start,planned_end,expected_start,expected_end,timezone,archived_at)",
        )
        .eq("tenant_id", tenant!.id)
      ...
      const [roleAssignments, staffAssignments] = await Promise.all([
        supabase
          ...
          .select(
            "participation_id,is_primary,operation_role_types(id,key,label,is_active,sort_order)",
          )
          .eq("tenant_id", tenant!.id)
      ...
      const rolesByParticipation = new Map<
        string,
        Array<{ id: string; label: string; isPrimary: boolean; sortOrder: number }>
      >();
      ...
      const allRoleIds = [
        ...new Set([...rolesByParticipation.values()].flat().map((role) => role.id)),
      ];
      ...
      for (const participation of activeParticipations) {
        ...
        if (schedules.length) {
          for (const schedule of schedules) {
            ...
            const roleLabel =
              scheduleRole?.label?.trim() || humanizeRoleKey(scheduleRole?.key || "role");
            const ownedItems = (playbooks.data ?? []).filter(
              (item) =>
                item.operation_id === participation.operation_id &&
                item.owner_role_type_id === schedule.role_type_id,
            );
            queue.push({
              ...
              pendingWork: ownedItems.filter((item) => latestAction.get(item.id) !== "completed")
                .length,
        ...
        const ownedItems = (playbooks.data ?? []).filter(
          (item) =>
            ...
            roleIds.has(item.owner_role_type_id),
        );
        queue.push({
      ...
      return queue.sort(
        (a, b) =>
          new Date(a.reportAt ?? a.start).getTime() - new Date(b.reportAt ?? b.start).getTime(),
      );
    },
  });
  ...
  if (query.isLoading) {
    return (
      <section
        ...
        aria-label={copy(locale, "Meu dia", "My day")}
        aria-busy="true"
      >
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-primary">
              COBS · {copy(locale, "Meu dia", "My day")}
            </p>
            <h3 className="mt-1 text-base font-semibold">
              {copy(
                locale,
                "Carregando sua jornada operacional…",
                "Loading your operational workday…",
              )}
            </h3>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {copy(
                locale,
                "Sua jornada não foi confirmada. Tente atualizar antes de assumir que não há compromissos.",
                "Your workday could not be confirmed. Refresh before assuming there are no assignments.",
              )}
            </p>
            <Button
              className="mt-3"
  ...
  const active = items.filter(
    (item) =>
      item.scheduleStatus !== "declined" &&
      item.scheduleStatus !== "completed" &&
      (item.operationStatus === "active" ||
        (new Date(item.start) <= now && new Date(item.end) >= now)),
  );
  const today = items.filter(
    (item) =>
      !active.some((activeItem) => activeItem.key === item.key) &&
      isSameLocalDay(new Date(item.reportAt ?? item.start), now),
  );
  const upcoming = items
    .filter(
      (item) =>
        !active.some((activeItem) => activeItem.key === item.key) &&
        !today.some((todayItem) => todayItem.key === item.key) &&
      ...
      <section className="surface-panel p-5" aria-label={copy(locale, "Meu dia", "My day")}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h3 className="mt-1 text-base font-semibold">
              {copy(locale, "Nenhuma operação atribuída", "No assigned operations")}
            </h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {copy(
                locale,
                "Quando você estiver vinculado a uma operação, sua jornada aparecerá aqui.",
                "When you are linked to an operation, your workday will appear here.",
              )}
            </p>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border bg-primary-soft/25 px-5 py-4">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-primary">
            PX12.4-D · {copy(locale, "Meu dia", "My day")}
          </p>
          <h3 className="mt-1 text-lg font-semibold">
            {copy(locale, "Sua jornada operacional", "Your operational workday")}
          </h3>
          <p className="mt-1 text-sm text-muted-foreground">
            {copy(
              locale,
              "Sua escala individual tem prioridade; operações antigas continuam funcionando por fallback.",
              "Your individual schedule has priority; legacy operations continue through fallback.",
            )}
          </p>
        </div>
        <span className="rounded-full border border-border bg-background px-3 py-1 text-xs text-muted-foreground">
          {items.length} {copy(locale, "compromisso(s)", "commitment(s)")}
        </span>
      </div>

      {statusMutation.isError ? (
        <div
          className="border-b border-destructive/25 bg-destructive/10 px-5 py-3 text-sm text-destructive"
          role="alert"
        >
          {copy(
            locale,
            "Não foi possível atualizar sua confirmação. Nenhuma mudança foi assumida; tente novamente.",
            "We couldn't update your confirmation. No change was assumed; try again.",
          )}
        </div>
      ) : null}
      ...
      <div className="space-y-5 p-5">
        ...
        {upcoming.length ? (
          <QueueGroup
            title={copy(locale, "Próximas", "Upcoming")}
            icon={CalendarClock}
            items={upcoming}
            locale={locale}
            fallbackTimeZone={timeZone}
  ...
  return (
    <div>
      <div className="mb-2 flex items-center gap-2">
        <Icon
          className={`size-4 ${emphasis ? "text-primary" : "text-muted-foreground"}`}
          aria-hidden="true"
        />
      ...
      <div className="grid gap-2">
        {items.map((item) => (
          <article
            key={item.key}
            className={`rounded-xl border p-4 transition-colors ${emphasis ? "border-primary/30 bg-primary-soft/20" : "border-border bg-background/50"}`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {item.scheduleStatus ? (
                    <SchedulePill status={item.scheduleStatus} locale={locale} />
                  ) : null}
                </div>
                <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.1em] text-muted-foreground">
                  {item.operationCode}
                  {item.primaryRole ? ` · ${item.primaryRole}` : ""}
                </p>
              </div>
              <Link
                to="/operations/$operationId"
                params={{ operationId: item.operationId }}
                aria-label={copy(locale, "Abrir operação", "Open operation")}
              >
                <ArrowRight
                  className="mt-1 size-4 shrink-0 text-muted-foreground hover:text-primary"
                  aria-hidden="true"
                />
              </Link>
            </div>
            ...
            <div className="mt-3 grid gap-2 text-xs text-muted-foreground sm:grid-cols-2">
              {item.reportAt ? (
                <span>
                  <strong className="text-foreground">
                    {copy(locale, "Apresentação", "Report")}
                  </strong>{" "}
                  ·{" "}
                  {formatDateTime(item.reportAt, {
                    locale,
              ...
              <span>
                <strong className="text-foreground">{copy(locale, "Trabalho", "Work")}</strong> ·{" "}
                {formatDateTime(item.start, {
                  locale,
                  timeZone: item.timezone || fallbackTimeZone,
                })}{" "}
                →{" "}
                {formatDateTime(item.end, { locale, timeZone: item.timezone || fallbackTimeZone })}
              </span>
              {item.totalWork > 0 ? (
                <span className="inline-flex items-center gap-1">
                  <CheckCircle2 className="size-3" aria-hidden="true" />
                  {item.pendingWork} {copy(locale, "pendência(s)", "pending")}
                </span>
              ) : null}
              {item.scheduleSource === "operation_fallback" ? (
                <span className="text-[10px] uppercase tracking-[0.08em]">
                  {copy(
                    locale,
                    "Horário da operação · sem escala individual",
                    "Operation window · no individual schedule",
                  )}
                </span>
              ) : null}
            </div>

            {item.roleLabels.length > 1 ? (
              <p className="mt-2 truncate text-[11px] text-muted-foreground">
                {copy(locale, "Funções", "Roles")}: {item.roleLabels.join(" · ")}
              </p>
            ) : null}

            {item.assignmentId && item.scheduleStatus === "assigned" ? (
              <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-border/70 pt-3">
                <span className="mr-1 text-xs font-medium text-warning">
                  {copy(
                    locale,
                    "Confirme sua participação nesta escala",
                    "Confirm your participation in this schedule",
                  )}
                </span>
                <Button
                  size="sm"
                  onClick={() =>
                    mutation.mutate({ assignmentId: item.assignmentId!, status: "confirmed" })
                  }
                ...
                >
                  ...
                  {mutation.isPending
                    ...
                    : copy(locale, "Confirmar", "Confirm")}
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() =>
                    mutation.mutate({ assignmentId: item.assignmentId!, status: "declined" })
                  }
                  disabled={mutation.isPending}
                >
                  <X className="mr-1 size-3.5" />
                  {copy(locale, "Recusar", "Decline")}
                </Button>
              </div>
            ) : null}
          </article>
...
function SchedulePill({ status, locale }: { status: ScheduleStatus; locale: string }) {
  ...
  return (
    <span
      className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${classes[status]}`}
    >
      {copy(locale, labels[status][0], labels[status][1])}
    </span>
  );
}
