...
export function OperationIntelligenceCockpit({ operationId }: { operationId: string }) {
  ...
  if (operation.isLoading)
    return (
      <div
        className="surface-panel h-36 animate-pulse"
        aria-label="Carregando cockpit operacional"
      />
    );
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-start gap-2 text-destructive">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Os dados atuais não puderam ser confirmados. Tente atualizar antes de tomar uma
                decisão operacional.
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/70 bg-primary-soft/30 px-5 py-4">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-primary">
            COBS Operational Intelligence
          </p>
          <h2 className="mt-1 text-xl font-semibold">Cockpit da operação</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Estado operacional consolidado em uma única visão.
          </p>
        </div>
      ...
      <div className="space-y-4 p-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Metric
            icon={Activity}
            label="Jornada"
            value={`${n(journey?.progress_percent)}%`}
            detail={`${n(journey?.completed_steps)}/${n(journey?.total_steps)} etapas concluídas`}
          />
          <Metric
            icon={Users}
            label="Viajantes"
            value={String(n(passengers?.confirmed))}
            detail={`${n(passengers?.current_step?.unresolved)} pendente(s) agora`}
          />
          <Metric
            icon={MessageSquare}
            label="Comunicação"
            value={`${n(data.communications?.read_rate_percent)}%`}
            detail={`${n(data.communications?.read)}/${n(data.communications?.recipients)} leituras`}
          />
          <Metric
            icon={WalletCards}
            label="Financeiro"
            value={money(data.commerce?.net_paid_minor, currency)}
            detail={`${money(data.commerce?.outstanding_minor, currency)} pendente`}
          />
        </div>
        ...
        <div className="rounded-xl border border-border/70 p-4">
          ...
          <div className="mt-2 grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <p className="font-medium">{journey?.next_step?.title ?? "Sem próxima etapa"}</p>
            </div>
          </div>
          <p className="mt-3 text-xs text-muted-foreground">
            {n(journey?.active_steps)} ativa(s) · {n(journey?.pending_steps)} pendente(s) ·{" "}
            {n(data.incidents?.total)} incidente(s)
          </p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Metric
            ...
            detail="deslocamentos que chegaram"
          />
          <Metric
            icon={BedDouble}
            label="Hospedagem"
            value={`${checkedIn}/${guests}`}
            detail={issues ? `${issues} pendência(s)` : `${stays.length} hospedagem(ns)`}
          />
          <Metric
            icon={CalendarDays}
            label="Eventos"
            value={`${n(data.events?.completed)}/${n(data.events?.total)}`}
            detail={`${n(data.events?.completed_sessions)}/${n(data.events?.total_sessions)} sessões concluídas`}
          />
          <Metric
            icon={Users}
            label="Presença efetiva"
            value={String(n(passengers?.effective_facts?.present))}
            detail={`${n(passengers?.effective_facts?.boarded)} embarcado(s) · ${n(passengers?.effective_facts?.no_show)} no-show`}
          />
        </div>
      </div>
    </section>
