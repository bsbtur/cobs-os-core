...
export function OperationAttentionCenter({ operationId }: { operationId: string }) {
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <LoadingPulse
            label={copy(
              locale,
              "Atualizando saúde operacional...",
              "Updating operational health...",
            )}
          />
        </div>
      </section>
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-warning">
                {copy(locale, "Central de atenção", "Attention center")}
              </p>
              <p className="mt-1 text-sm font-semibold">
                {copy(
                  locale,
                  "Não foi possível atualizar os sinais operacionais",
                  "Could not refresh operational signals",
                )}
              </p>
              <p className="mt-0.5 text-xs text-muted-foreground">
                {copy(
                  locale,
                  "As tentativas automáticas falharam. Não interprete a ausência de alertas como operação saudável até uma atualização bem-sucedida.",
                  "Automatic retries failed. Do not treat missing alerts as a healthy operation until a refresh succeeds.",
                )}
              </p>
            </div>
          </div>
  ...
  const healthReasons = (data.health?.reasons ?? []).filter(
    (reason) => !isCurrentStepPassengerBlocker(reason.code),
  );
  ...
  for (const reason of healthReasons) {
    ...
    attentions.push({
      severity:
        reason.severity === "critical" || data.health?.level === "red" ? "critical" : "warning",
      title: humanizeCode(reason.code, locale),
      detail:
        reason.value == null
          ? copy(
              locale,
              "Condição sinalizada pelo Intelligence Core.",
              "Condition flagged by the Intelligence Core.",
            )
          : copy(locale, `Valor observado: ${reason.value}.`, `Observed value: ${reason.value}.`),
    });
  }

  const incidents = n(data.incidents?.total);
  if (incidents > 0 && !healthReasons.some((r) => r.code?.includes("INCIDENT")))
    attentions.push({
      severity: "critical",
      title: copy(locale, "Incidentes operacionais", "Operational incidents"),
      detail: copy(
        locale,
        `${incidents} incidente(s) registrado(s).`,
        `${incidents} incident(s) recorded.`,
      ),
    });

  const hotelIssues = (data.hospitality?.stays ?? []).reduce(
    (sum, stay) => sum + n(stay.issues),
    0,
  );
  if (hotelIssues > 0)
    attentions.push({
      severity: "warning",
      title: copy(locale, "Pendências de hospedagem", "Hospitality issues"),
      detail: copy(
        locale,
        `${hotelIssues} pendência(s) precisam de atenção.`,
        `${hotelIssues} issue(s) need attention.`,
      ),
    });
  ...
  if (urgentUnread > 0)
    attentions.push({
      severity: "critical",
      title: copy(locale, "Alerta urgente não lido", "Unread urgent alert"),
      detail: copy(
        locale,
        `${urgentUnread} leitura(s) urgente(s) ainda pendentes.`,
        `${urgentUnread} urgent read(s) are still pending.`,
      ),
    });

  const delay = n(data.journey?.delay?.minutes);
  if (delay > 0 && !healthReasons.some((r) => r.code?.includes("DELAY")))
    attentions.push({
      severity: delay >= 15 ? "critical" : "warning",
      title: copy(locale, "Atraso operacional", "Operational delay"),
      detail: copy(locale, `A etapa está ${delay} min atrasada.`, `The step is ${delay} min late.`),
    });
        ...
        <div className="flex items-start gap-2.5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-success">
              {copy(locale, "Nenhuma atenção crítica agora", "No critical attention needed now")}
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 space-y-2">
        {attentions.slice(0, 3).map((item, index) => (
          <AttentionRow key={`${item.title}-${index}`} item={item} />
        ))}
      </div>
      {attentions.length > 3 ? (
        <p className="mt-2 text-[11px] text-muted-foreground">
          +{attentions.length - 3} {copy(locale, "outro(s) sinal(is)", "more signal(s)")}
        </p>
      ) : null}
    </section>
  );
...
function AttentionRow({ item }: { item: Attention }) {
  const Icon =
    item.severity === "critical" ? AlertCircle : item.severity === "warning" ? AlertTriangle : Info;
  const classes =
    item.severity === "critical"
      ? "border-destructive/30 bg-destructive/10 text-destructive"
      : item.severity === "warning"
        ? "border-warning/30 bg-warning-soft text-warning"
        : "border-primary/20 bg-primary-soft/30 text-foreground";
  return (
    <div className={`flex items-start gap-2.5 rounded-lg border px-3 py-2 ${classes}`}>
