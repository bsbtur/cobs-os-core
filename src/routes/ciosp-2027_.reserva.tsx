  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (checkoutError) {
        ...
        if (code === "qa_identity_required") {
          setError("No modo QA, use um nome terminado em QA e um e-mail @example.com.br.");
          return;
        }
        if (code === "qa_preview_not_allowed") {
          setError("Este modo QA só pode ser executado no Preview autorizado do COBS.");
          return;
        }
        if (
  ...
  return (
    <main className="min-h-screen bg-[#070706] px-5 py-10 text-white sm:px-8 sm:py-14">
      <section className="mx-auto max-w-xl">
        ...
        >
          <ArrowLeft className="size-4" />Voltar para CIOSP 2027
        </a>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-white/55">
                Valor aprovado: R$ 12.490 por passageiro em acomodação dupla. Entrada de R$ 3.490 + 3 parcelas de R$ 3.000, com vencimentos em 10/10/2026, 10/11/2026 e 10/12/2026.
              </p>
            </div>
          </div>
          {pix ? (
            ...
            <form onSubmit={submit} className="mt-8 space-y-4">
              ...
              <label className="flex items-start gap-3 rounded-xl border border-white/10 bg-black/30 p-4 text-sm leading-6 text-white/65">
                <input required type="checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} className="mt-1 size-4" />
                <span>Li e aceito os <a href="/termos-ciosp-2027" target="_blank" rel="noreferrer" className="font-semibold text-[#E4CA91] underline underline-offset-4">Termos Comerciais e Política de Cancelamento</a> da CIOSP Experience 2027 e autorizo o uso dos meus dados para esta contratação.</span>
              </label>
              <p className="text-xs leading-5 text-white/45">Consulte também o <a href="/privacidade-ciosp-2027" className="font-semibold text-[#E4CA91] underline underline-offset-4">Aviso de Privacidade</a>.</p>
              {closed && <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm leading-6 text-amber-200" role="status">As vendas ainda estão em validação final. Nenhuma cobrança foi criada por esta tentativa.</div>}
              ...
              <Button type="submit" size="lg" className="w-full bg-[#D6B56D] text-black hover:bg-[#E4CA91]" disabled={loading || !consent}>
                {loading ? <><Loader2 className="mr-2 size-4 animate-spin" />Preparando reserva...</> : "Gerar reserva e Pix"}
              </Button>
            </form>
          )}
        </div>
