    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data, error } = await supabaseAdmin.rpc("record_provider_payment_event", {
          ...
          _event_type: eventType as never,
          _external_reference: body.data.external_reference ?? undefined,
          _provider_payment_id: body.data.id ?? undefined,
          _provider_event_id: body.id ?? undefined,
          _provider_status: status,
          _provider_status_detail: body.data.status_detail ?? undefined,
          _amount: body.data.transaction_amount ?? undefined,
