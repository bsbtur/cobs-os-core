...
type Assignment = {
  ...
  operation_participations?: {
    people?: { id?: string | null; full_name?: string | null } | null;
  } | null;
...
function TeamSchedulePage() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const candidate = query.data?.candidates.find(
        (item) => `${item.participationId}:${item.roleTypeId}` === form.candidate,
      );
      if (!candidate || !tenant?.id || !form.startsAt || !form.endsAt) {
        throw new Error(
          copy(
            locale,
            "Preencha profissional, função, início e fim.",
            "Fill professional, role, start and end.",
          ),
        );
      }
      ...
      if (!Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime()) || end <= start) {
        throw new Error(
          copy(
            locale,
            "O fim da escala precisa ser posterior ao início.",
  ...
  const cancelAssignment = (id: string) => {
    if (
      query.data?.operation?.status === "completed" ||
      query.data?.operation?.status === "cancelled"
    )
      return;
  ...
  if (query.isLoading)
    return (
      <div className="surface-panel p-5 text-sm text-muted-foreground">
        {copy(locale, "Carregando escala…", "Loading schedule…")}
      </div>
    );
        ...
        <div className="flex items-start gap-3 text-destructive">
          ...
          <div>
            <p className="font-semibold">
              {copy(locale, "Não foi possível carregar a escala.", "Could not load schedule.")}
            </p>
            <p className="mt-1 text-sm text-muted-foreground">
              {copy(
                locale,
                "Os dados da equipe não foram confirmados. Tente atualizar antes de assumir que não há profissionais escalados.",
                "Team data could not be confirmed. Retry before assuming no professionals are scheduled.",
              )}
            </p>
            <Button
              className="mt-3"
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-primary">
              PX12.4 · Team Schedule
            </p>
            <h2 className="mt-1 text-xl font-semibold">
              {copy(locale, "Escala da equipe", "Team schedule")}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
      ...
      {operationClosed ? (
        <section className="surface-panel px-4 py-3 text-sm text-muted-foreground">
          ...
          {copy(
            locale,
            "A escala está disponível somente para consulta histórica.",
            "The schedule is available for historical review only.",
          )}
        </section>
      ) : null}
      ...
      {conflicts.length ? (
        <section className="rounded-xl border border-destructive/30 bg-destructive/10 p-4 text-destructive">
          <div className="flex items-center gap-2 font-semibold">
            ...
            {copy(locale, "Conflito de escala detectado", "Schedule conflict detected")}
          </div>
          <p className="mt-1 text-sm opacity-85">
            {copy(
              locale,
              "Há sobreposição ativa. Revise os horários antes da operação.",
              "There is an active overlap. Review times before the operation.",
            )}
          </p>
        </section>
      ) : null}
      ...
      {canManage && !operationClosed ? (
        <section className="surface-panel p-5">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {copy(
              locale,
              "Somente pessoas e funções já vinculadas à operação aparecem aqui.",
              "Only people and roles already linked to the operation appear here.",
            )}
          </p>
          {noCandidates ? (
            <div className="mt-4 rounded-lg border border-warning/30 bg-warning-soft p-3">
              <p className="text-sm font-semibold text-warning">
                {copy(
                  locale,
                  "Nenhum profissional elegível para escalar",
                  "No eligible professional to schedule",
                )}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {copy(
                  locale,
                  "Vincule primeiro uma pessoa e uma função ativa à operação; depois ela aparecerá aqui.",
                  "First link a person and an active role to the operation; then it will appear here.",
                )}
              </p>
            </div>
          ) : null}
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            <label className="space-y-1 text-xs font-medium">
              ...
              <select
                ...
                disabled={noCandidates || actionsPending}
                onChange={(e) => setForm((v) => ({ ...v, candidate: e.target.value }))}
              >
                <option value="">{copy(locale, "Selecione…", "Select…")}</option>
                {candidates.map((item) => (
                  <option
                    key={`${item.participationId}:${item.roleTypeId}`}
                    value={`${item.participationId}:${item.roleTypeId}`}
                  >
                    {item.personName} · {item.roleLabel}
                  </option>
                ))}
              </select>
            </label>
            <Field
              label={copy(locale, "Apresentação", "Report at")}
              value={form.reportAt}
              onChange={(value) => setForm((v) => ({ ...v, reportAt: value }))}
            />
            <Field
              label={copy(locale, "Início", "Start")}
              value={form.startsAt}
              onChange={(value) => setForm((v) => ({ ...v, startsAt: value }))}
            />
            <Field
              label={copy(locale, "Fim", "End")}
              value={form.endsAt}
              onChange={(value) => setForm((v) => ({ ...v, endsAt: value }))}
            />
            <label className="space-y-1 text-xs font-medium md:col-span-2">
              <span>{copy(locale, "Observações", "Notes")}</span>
              <Input
                value={form.notes}
                onChange={(e) => setForm((v) => ({ ...v, notes: e.target.value }))}
                placeholder={copy(
                  locale,
                  "Ex.: chegar uniformizado na Torre de TV",
                  "E.g. arrive in uniform at meeting point",
                )}
              />
            </label>
          </div>
          <Button
            className="mt-4"
            onClick={() => save.mutate()}
            disabled={actionsPending || noCandidates}
          >
            {save.isPending
              ? copy(locale, "Salvando…", "Saving…")
              : copy(locale, "Adicionar escala", "Add schedule")}
          </Button>
        </section>
      ) : null}
      ...
      <section className="surface-panel overflow-hidden">
        <div className="border-b border-border px-5 py-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {assignments.length} {copy(locale, "atribuição(ões)", "assignment(s)")}
          </p>
        </div>
        {assignments.length ? (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <p className="font-semibold">
                          {person?.full_name ?? copy(locale, "Profissional", "Professional")}
                        </p>
                        <Status status={assignment.status} locale={locale} />
                        {conflicted ? (
                          <span className="rounded-full bg-destructive/10 px-2 py-0.5 text-[10px] font-semibold text-destructive">
                            {copy(locale, "CONFLITO", "CONFLICT")}
                          </span>
                        ) : null}
                      </div>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {roleType?.label || humanize(roleType?.key || "role")}
                      </p>
                    </div>
                    <div className="text-right text-xs text-muted-foreground">
                      {assignment.report_at ? (
                        <p>
                          {copy(locale, "Apresentação", "Report")}:{" "}
                          <strong className="text-foreground">
                            {formatDate(assignment.report_at)}
                          </strong>
                        </p>
                      ) : null}
                      <p>
                        {copy(locale, "Trabalho", "Work")}:{" "}
                        <strong className="text-foreground">
                          {formatDate(assignment.starts_at)} → {formatDate(assignment.ends_at)}
                        </strong>
                      </p>
                    </div>
                  </div>
                  {assignment.notes ? (
                    <p className="mt-2 text-xs text-muted-foreground">{assignment.notes}</p>
                  ) : null}
                  <div className="mt-3 flex flex-wrap gap-2">
                    {!operationClosed && own && assignment.status === "assigned" ? (
                      <>
                        <Button
                          size="sm"
                    ...
                    ["assigned", "confirmed"].includes(assignment.status) ? (
                      ...
                      >
                        {statusMutation.isPending
                          ? copy(locale, "Atualizando…", "Updating…")
                          : copy(locale, "Cancelar escala", "Cancel assignment")}
                      </Button>
                    ) : null}
                  </div>
                </article>
          ...
          <div className="p-8 text-center text-sm text-muted-foreground">
            <UserCheck className="mx-auto mb-2 size-5" />
            {copy(locale, "Nenhum profissional escalado ainda.", "No professionals scheduled yet.")}
          </div>
        )}
      </section>
      ...
      <p className="text-xs text-muted-foreground">
        ...
        {copy(
          locale,
          "Horários são gravados como timestamptz; a edição usa o fuso do dispositivo nesta V1.",
          "Times are stored as timestamptz; editing uses the device timezone in this V1.",
        )}
      </p>
      <Button asChild variant="ghost">
        <Link to="/operations/$operationId" params={{ operationId }}>
          {copy(locale, "Voltar à visão geral", "Back to overview")}
        </Link>
      </Button>
    </div>
  );
...
function Status({ status, locale }: { status: AssignmentStatus; locale: string }) {
  ...
  return (
    <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">
      {copy(locale, label[status][0], label[status][1])}
    </span>
  );
}
