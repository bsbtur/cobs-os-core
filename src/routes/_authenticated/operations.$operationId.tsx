...
function TerminalLiveRecord({
  ...
  const events = useQuery({
    queryKey: ["terminal-live-events", operationId],
    queryFn: async () => {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="surface-panel border-success/40 px-4 py-4">
        <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-success">
          {completed ? "Operação concluída" : "Operação cancelada"}
        </p>
        <h3 className="mt-2 text-lg font-semibold">Jornada concluída</h3>
        <p className="mt-1 text-sm text-muted-foreground">
          Registro operacional disponível somente para consulta. Nenhuma ação operacional pode ser
          registrada após o encerramento.
        </p>
      </div>
            ...
            {(events.data ?? []).map((event) => (
              <li key={event.id} className="flex flex-wrap items-baseline gap-2 text-sm">
                <span className="font-mono text-xs tabular-nums text-muted-foreground">
                  {formatDateTime(event.occurred_at, { locale, timeZone: timezone })}
                </span>
...
function OperationWorkspace() {
  ...
  const operation = useQuery({
    queryKey: ["operation-workspace-status", operationId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("operations")
