...
describe("PILOT-03 post-completion hardening", () => {
  ...
  test("server guard covers staff assignment updates", () => {
    const s = read("supabase/migrations/20260815211500_px12_7_closed_operation_read_only_guards.sql");
  ...
  test("overview intelligence query and polling use canonical capability", () => {
    ...
    expect(s).toContain("const liveHealthEnabled = canUseLiveOperationalHealth(operation.data?.status)");
  ...
  test("completed intelligence renders neutral historical state without retry CTA", () => {
    ...
    expect(historical).toContain("Os indicadores operacionais em tempo real não são mais atualizados");
